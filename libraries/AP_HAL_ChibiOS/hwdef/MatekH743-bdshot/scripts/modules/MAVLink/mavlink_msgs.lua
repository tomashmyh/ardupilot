...
function mavlink_msgs.decode_header(message)
  ...
  local bytes_to_unpack = "<I"
  if result.protocol_version == 2 then
    bytes_to_unpack = "<I3"
  end

  result.msgid, read_marker = string.unpack(bytes_to_unpack, message, read_marker)
...
function mavlink_msgs.decode(message, msg_map)
  ...
  local name = msg_map[result.msgid]
  if not name then
    return nil
  end
  local message_map = require("MAVLink/mavlink_msg_" .. name)
  if not message_map then
