...
void NavEKF3_core::readGpsData()
{
    ...
    if ((frontend->_options & (int32_t)NavEKF3::Options::JammingExpected) &&
        ...
        const bool forceDeadReckonWhileUnderJamming = (frontend->_options & (int32_t)NavEKF3::Options::ForceDeadReckonWhileUnderJamming);
        const bool canDoWindRelNav = assume_zero_sideslip();
        const bool canDeadReckon = ((doingFlowNav && gndOffsetValid) || canDoWindRelNav || doingBodyVelNav || forceDeadReckonWhileUnderJamming);
        if (canDeadReckon) {
